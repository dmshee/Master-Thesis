...
load('MIX_SWIR.mat'); % Load the MIX_SWIR data

mean_image = mean(im_reflectance, 3);
figure, imshow(mean_image, [])
title('Mean Image Across All Wavelength Bands for Mix Reflectance')
...
figure;
imshow(pseudoRGB);
title('Pseudo-RGB Image from Selected Hyperspectral Bands');
...
for sampleID = 1:25
    ...
    sampleMean_SWIR(sampleID).sampleID = sampleID;
    sampleMean_SWIR(sampleID).sampleName = sprintf('%s_%02d', classNames{classID}, sampleID);
    sampleMean_SWIR(sampleID).classID = classID;
    sampleMean_SWIR(sampleID).className = classNames{classID};
    sampleMean_SWIR(sampleID).wavelengths = WL;
    sampleMean_SWIR(sampleID).meanSpectrum = meanSpectrum;
    sampleMean_SWIR(sampleID).unfoldedSpectra = sampleSpectra;
    sampleMean_SWIR(sampleID).nPixels = size(sampleSpectra, 1);
end


% saving the sample mean spectra and class map for later use
save('MIX_SWIR_sampleMean.mat', 'sampleMean_SWIR', 'classMap', 'WL');
...
%% Preprocessing on raw sample spectra
X_raw = sample_mean_spectra;

% Apply SNV and MSC to the raw sample spectra
X_snv = SNV(X_raw);
X_msc = MSC_reworked(X_raw, mean(X_raw, 1));

% Plot class means for each version
...
for classID = 1:numel(classNames)
    ...
    class_mean_raw = mean(X_raw(sampleIDs, :), 1);
    plot(WL, class_mean_raw, 'LineWidth', 2)
end
hold off
grid on
xlabel('Wavelength (nm)')
ylabel('Mean Reflectance')
title('Mean Spectral Profiles by Class (Raw)')
...
for classID = 1:numel(classNames)
    sampleIDs = classSamples(classID, :);
    class_mean_snv = mean(X_snv(sampleIDs, :), 1);
...
for classID = 1:numel(classNames)
    sampleIDs = classSamples(classID, :);
    class_mean_msc = mean(X_msc(sampleIDs, :), 1);
...
% Overlay Raw, SNV, and MSC on one graph
figure('Position', [100 100 1200 700]);
hold on
grid on

dataSets = {X_raw, X_snv, X_msc};
prepNames = {'Raw', 'SNV', 'MSC'};
lineStyles = {'-', '--', ':'};
classColors = lines(numel(classNames));

for classID = 1:numel(classNames)
    sampleIDs = classSamples(classID, :);

    for p = 1:numel(dataSets)
        classMean = mean(dataSets{p}(sampleIDs, :), 1);

        plot(WL, classMean, ...
            'Color', classColors(classID, :), ...
            'LineStyle', lineStyles{p}, ...
            'LineWidth', 1.8, ...
            'DisplayName', sprintf('%s - %s', classNames{classID}, prepNames{p}));
    end
end

hold off
xlabel('Wavelength (nm)')
ylabel('Reflectance / Normalized Reflectance')
title('Class Means Overlay: Raw vs SNV vs MSC')
